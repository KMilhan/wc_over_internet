...
from simplewc.model import raise_if_not_safe, HTMLDocumentModel
from simplewc.storage import MockDocumentStorage, MockQueryCache

LINK_TO_VERY_BIG_RESOURCE = 'http://ftp.riken.jp/Linux/ubuntu-releases/18.04' \
...
def test_raise_if_local():
    ...
    with pytest.raises(NotAllowedScheme):
        HTMLDocumentModel('ftp://google.com/index.html',
                          mock_doc_storage,
                          mock_query_cache
                          )

    with pytest.raises(NotAllowedScheme):
        HTMLDocumentModel('file:///etc/resolv.conf.d/resolv.conf',
                          mock_doc_storage,
                          mock_query_cache
                          )

    with pytest.raises(AccessLocalURI):
        HTMLDocumentModel('https://127.0.0.1/index.html',
                          mock_doc_storage,
                          mock_query_cache
                          )
